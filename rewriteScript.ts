    ...
    if (await isExtensionEnabled()) {
                ...
                checkStatus(lcLink).then(async (problemStatus) => {
                    if (!problemStatus) {
                        ...
                        setTimeout(async () => {
                            ...
                            if (isAccepted) {
                                probSolved++
                                console.log("Problem accepted, updating status and redirecting if needed")
        ...
        function addWebRequestListener() {
            ...
            if (!webRequestListenerAdded) {
                chrome.webRequest.onCompleted.addListener(webRequestListener, { urls: ["<all_urls>"] }, ["responseHeaders"])
        ...
        async function setStatusFalse() {
            ...
            try {
                ...
                console.log(`Status set to false for problems ${false_status_count} to ${endIndex - 1}`)
